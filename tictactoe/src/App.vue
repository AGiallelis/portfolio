        ...
        <div class="flex flex-col items-center mb-8">
          <div
            ...
            <div 
                ...
                text-4x1 cursor pointer`">
                {{ cell=== 'X' ? `close` : cell === 'O' ? 'circle': ''}}
            </div>
          </div>
        </div>
      <h2 v-if="winner" class="text-6xl fond-bold mb-8">Player '{{winner}}' wins!</h2>
      <button @click="ResetGame" class="px-4 py-2 bg-pink-500 rounded uppercase font-bold hover:big-pink-600 duration-300">Reset Game</button>
  </main>
</template>
